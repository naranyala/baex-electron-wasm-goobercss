...
/**
 * A Kanban board component that demonstrates WASM-managed state synchronization.
 * Users can view tasks organized into columns and move them through the workflow.
 */
export class KanbanComponent extends ExbaComponent {
  /** Enables Shadow DOM for style encapsulation. */
  static useShadow = true;
  
  /** Scoped CSS styles for the Kanban board and its cards. */
  static styles = {
  ...
  /** Fetches initial task data from WASM on mount. */
  protected async onMount() {
    this.refresh();
  }

  /**
   * Dispatches a KanbanFetch IR command to Rust to retrieve current tasks.
   */
  ...
  /**
   * Dispatches a MoveTask IR command to Rust to cycle a task's column.
  ...
  /**
   * Renders the Kanban board with three columns: To Do, In Progress, and Done.
   */
  render() {
  ...
  /**
   * Registers a global dispatch function for task movement to handle onclick events 
   * from the rendered HTML string.
   */
  connectedCallback() {
