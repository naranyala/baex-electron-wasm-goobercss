...
/**
 * A bottom-sheet drawer component that slides in from the bottom of the screen.
 * Provides a modal-like experience with a background backdrop blur and 
 * physics-based slide animations.
 */
export class DrawerComponent extends ExbaComponent {
  /** Uses Shadow DOM to isolate the fixed-position drawer and backdrop. */
  static useShadow = true;
  
  /** Scoped styles for the drawer, backdrop, and hardware-accelerated transitions. */
  static styles = {
    container: 'padding: 2rem; width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 400px;',
  ...
  /** Initializes the drawer in a closed state. */
  protected onMount() {
    this.setState({ isOpen: false });
  }

  /** Sets the drawer state to open. */
  public open() { this.setState({ isOpen: true }); }
  /** Sets the drawer state to closed. */
  public close() { this.setState({ isOpen: false }); }

  /**
   * Renders the trigger button, the backdrop, and the sliding drawer panel.
   */
  render() {
  ...
  /** Registers global helper functions for external control. */
  connectedCallback() {
