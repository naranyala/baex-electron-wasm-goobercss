...
/**
 * A settings panel component with interactive toggle switches.
 * Demonstrates boolean property handling and direct state mutation through
 * inline event handlers in the rendered HTML.
 */
export class SettingsComponent extends ExbaComponent {
  /**
   * Observed configuration properties:
   * - darkMode: Boolean toggle for dark mode.
   * - notifications: Boolean toggle for alerts.
   * - autoUpdate: Boolean toggle for background updates.
   */
  ...
  /** Inject settings-specific toggle and layout styles. */
  static styles = STYLES;

  /**
   * Renders the settings list with reactive toggle switches.
   */
  render() {
    const {
      darkMode = true,
