 ...
 * A minimal tagged template literal system for targeted DOM patching.
 */
export interface TemplateNode {
  type: 'static' | 'dynamic';
  content: string | ((state: any) => string);
}
...
export function html(strings: TemplateStringsArray, ...values: any[]) {
  ...
  for (let i = 0; i < strings.length; i++) {
    if (strings[i]) {
      nodes.push({ type: 'static', content: strings[i] });
    }
    if (i < values.length) {
      nodes.push({ type: 'dynamic', content: (state: any) => String(values[i]) });
    }
  }
  return nodes;
}
 ...
 */
export function createPortal<T extends HTMLElement>(
  target: HTMLElement,
