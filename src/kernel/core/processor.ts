...
export class IRProcessor {
  ...
  static process(bundle: any) {
    ...
    // 0. Process Meta-Commands (Meta-Layer Integration)
    if (validatedBundle.meta) {
      validatedBundle.meta.forEach((meta: any) => {
        IRProcessor.executeMeta(meta);
      });
    }
  ...
  /**
   * Executes a meta-command sent from the Rust core.
   * This bridges the Rust engine directly into the TS Meta-Layer (Plugins/Interceptors).
   * @param meta The meta-command object.
   */
  private static executeMeta(meta: any) {
    const { action, plugin_target, payload } = meta;
    EXBA.log('IR_META_EXEC', { action, plugin_target });

    // 1. Dispatch as a global framework event so plugins can listen
    EXBA.emit(`meta:${action}`, { 
      plugin_target, 
      payload, 
      timestamp: Date.now() 
    });

    // 2. If a specific plugin is targeted, we can also update its isolated state
    if (plugin_target) {
      const state = EXBA.getPluginState(plugin_target);
      if (state) {
