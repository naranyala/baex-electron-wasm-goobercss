...
import { customElement, property, state, wasmState, wasmCall, wasmCommand, eventListener } from '../core/decorators';
...
describe('ExbaComponent Advanced Behaviors', () => {
  ...
  describe('Decorators', () => {
    it('should register component and define props using customElement and property decorators', () => {
      @customElement('dec-comp')
      class DecComp extends ExbaComponent {
        @property('number') age!: number;
        @property('string') name!: string;
        @state() count = 0;

        render() {
          return `<div>${this.name}: ${this.age} (clicks: ${this.count})</div>`;
        }
      }

      expect(DecComp).toBeDefined();

      const el = document.createElement('dec-comp') as any;
      document.body.appendChild(el);

      el.setAttribute('name', 'Antigravity');
      el.setAttribute('age', '5');

      expect(el.state.name).toBe('Antigravity');
      expect(el.state.age).toBe(5);

      // Verify state decorator reactivity
      expect(el.count).toBe(0);
      el.count = 10;
      expect(el.count).toBe(10);
      expect(el.state.count).toBe(10);

      expect(el.shadowRoot.innerHTML).toContain('Antigravity: 5 (clicks: 10)');

      el.remove();
    });

    it('should sync properties and proxy calls to WASM using wasmState, wasmCall, and wasmCommand', async () => {
      const mockState = {
        value: {
      ...
      const mockApi = {
        ...
        compile_ir: vi.fn().mockResolvedValue(new Uint8Array([1, 2, 3])),
        execute_bytecode: vi.fn().mockResolvedValue({ type: 'Number', payload: 100 }),
      };

      EXBA.state = mockState as any;
      (EXBA as any)._api = mockApi;

      @customElement('wasm-dec-comp')
      class WasmDecComp extends ExbaComponent {
        @wasmState('userName') user!: string;

        @wasmCall()
        add!: (a: number, b: number) => Promise<number>;

        @wasmCommand('Fact')
        fact!: (n: number) => Promise<any>;
      ...
      expect(WasmDecComp).toBeDefined();

      const el = document.createElement('wasm-dec-comp') as any;
      document.body.appendChild(el);

      expect(el.user).toBe('Antigravity');
      el.user = 'NewDeveloper';
      expect(mockState.value.userName).toBe('NewDeveloper');

      const addRes = await el.add(10, 20);
      expect(addRes).toBe(42);
      expect(mockApi.add).toHaveBeenCalledWith(10, 20);

      const factRes = await el.fact(5);
      expect(factRes).toEqual({ type: 'Number', payload: 100 });
      expect(mockApi.compile_ir).toHaveBeenCalled();
      expect(mockApi.execute_bytecode).toHaveBeenCalled();

      el.remove();
      EXBA.state = null;
      (EXBA as any)._api = null;
    });

    it('should bind event listeners using eventListener decorator', async () => {
      const clickSpy = vi.fn();

      @customElement('event-dec-comp')
      class EventDecComp extends ExbaComponent {
        @state() count = 0;
        ...
        render() {
          return `<div><button id="btn">Clicked: ${this.count}</button></div>`;
        }
      }

      const el = document.createElement('event-dec-comp') as any;
      document.body.appendChild(el);

      // Wait for event listener to be bound in the microtask
      await Promise.resolve();

      const btn = el.shadowRoot.querySelector('#btn');
      expect(btn).toBeDefined();

      btn.dispatchEvent(new CustomEvent('click', { bubbles: true, composed: true }));
      expect(clickSpy).toHaveBeenCalledTimes(1);
      expect(el.count).toBe(1);
      expect(el.shadowRoot.innerHTML).toContain('Clicked: 1');

      // Wait for the update cycle to complete and bind listeners again
      await Promise.resolve();

      const newBtn = el.shadowRoot.querySelector('#btn');
      newBtn.dispatchEvent(new CustomEvent('click', { bubbles: true, composed: true }));
    ...
    it('should reconcile keyed list elements using key-based diffing in dom.ts', () => {
      @customElement('keyed-list-comp')
      class KeyedListComp extends ExbaComponent {
        @state() items = [
          { id: '1', name: 'Task 1' },
          { id: '2', name: 'Task 2' },
          { id: '3', name: 'Task 3' }
        ...
        render() {
          return `
            <ul>
              ${this.items.map(item => `<li key="${item.id}" id="item-${item.id}">${item.name}</li>`).join('')}
            </ul>
          `;
        }
      }

      const el = document.createElement('keyed-list-comp') as any;
      document.body.appendChild(el);

      // Save initial node references
      const item1 = el.shadowRoot.querySelector('#item-1');
      const item2 = el.shadowRoot.querySelector('#item-2');
      const item3 = el.shadowRoot.querySelector('#item-3');
      ...
      // Check new positions and reference preservation
      const newItem3 = el.shadowRoot.querySelector('#item-3');
      const newItem1 = el.shadowRoot.querySelector('#item-1');
      const newItem2 = el.shadowRoot.querySelector('#item-2');

      expect(newItem2).toBeNull(); // Removed
      expect(newItem3).toBe(item3); // Same node reference preserved!
      expect(newItem1).toBe(item1); // Same node reference preserved!

      const lis = el.shadowRoot.querySelectorAll('li');
      expect(lis[0]).toBe(item3); // Node 3 is now first
      expect(lis[1]).toBe(item1); // Node 1 is now second
