...
export class IRValidator {
  ...
  static validate(command: any): asserts command is HLIRCommand {
    ...
    const validTypes = [
      ...
      'PalindromeCheck', 'Greet', 'ReportAnomaly', 'RulesQuery', 'SystemFetch'
    ];
    ...
    switch (command.type) {
      ...
      case 'RulesQuery':
        break;
      case 'SystemFetch':
        if (typeof command.payload?.snapshot !== 'string') {
          throw new Error('IR Error: "SystemFetch" requires a string snapshot in payload.');
        }
        break;
    }
  }
