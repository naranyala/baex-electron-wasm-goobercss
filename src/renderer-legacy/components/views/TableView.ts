...
export const TableView = defineComponent({
  ...
  events: {
    ...
    'input [data-action="update-field"]': (e: Event, { state, setState }: any) => {
        ...
        <table class="${styles.table}">
          <thead>
            <tr style="text-align: left;">
              ${columns.map(col => `<th class="${styles.th}">${col}</th>`)}
          ...
          <tbody>
            ${state.data.map((row: any) => `
              <tr class="${state.editingRowId === row.rowid ? styles.trEditing : ''}">
                ${columns.map(col => `<td class="${styles.td}">${row[col]}</td>`)}
          ...
          ${columns.map(col => `
            <div class="${styles.formField}">
              ...
              <input type="text" value="${state.formValues[col] || ''}" 
                     data-col="${col}" data-action="update-field" class="${styles.formInput}" />
            </div>
          `)}
        </div>
